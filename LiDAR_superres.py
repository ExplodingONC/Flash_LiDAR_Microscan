...
try:
    ...
    for multi_frame in range(5):
        ...
        # log data
        if multi_frame > 0:
            sigs.append(SensorSignal.SensorSignal([height, width], downsample_ratio=2))
            sigs[-1].use_data(data)
            sigs[-1].set_timing(T0_pulse_time)
            sigs[-1].shift_vector = np.array([(multi_frame - 1) % 2, (multi_frame - 1) // 2]) / 2
